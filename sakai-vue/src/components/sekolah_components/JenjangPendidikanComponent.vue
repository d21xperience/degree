<script setup>
import { useEducationStages } from '@/composables/sekolah_composable/useEducationStages';
import { computed, onMounted, ref } from 'vue';

//  Ambil API lengkap — termasuk `initialize` dan state loading/error
const { initialize, isFetching, data, error } = useEducationStages({ autoload: false }); // ❗ nonaktifkan autoload agar kita kendalikan manual
...
const initComponent = async () => {
    try {
        ...
        jenjangPendidikanOptions.value = data.value;
    } catch (err) {
        console.error('[JenjangPendidikanComponent] Gagal inisialisasi:', err);
    }
};
...
<template>
    <div class="flex min-w-72 items-center space-x-2">
        ...
        <template v-else>
            <Select v-model="internalValue" :options="jenjangPendidikanOptions" option-label="nama" option-value="nama" placeholder="Pilih jenjang pendidikan" fluid :disabled="props.isDisabled || jenjangPendidikanOptions.length === 0" class="w-36" />
        </template>
    </div>
</template>
