        ...
        <Dialog v-model:visible="isAddSemester" :style="{ width: '450px' }" :header="judul" :modal="true" position="top" @hide="resetValue">
            <div class="grid gap-y-2">
                ...
                <div>
                    ...
                    <DatePicker v-model="semester.tahunAjaranId" view="year" dateFormat="yy" :inline="false" fluid />
                </div>
...
const handleCreateSemester = () => {
    ...
    if (semester.tanggalMulai && semester.tanggalSelesai) {
        try {
            // Simulasi data baru
            const newSemester = {
                semesterId: Date.now(), // ID unik
                namaSemester: semester.namaSemester || '2025/2026',
            ...
            semesterList.value.push(newSemester); // ✅ tambah ke list
...
const handleDeleteSemester = async () => {
    try {
        ...
        const res = await sekolahService.deleteSemester(semesterDelete);
        if (res.status) {
...
const handleUpdateSemester = async () => {
    try {
        ...
        const res = await sekolahService.updateSemester(originalSemesterList);
...
const initial = async () => {
    ...
    try {
        semesterList.value = await sekolahService.fetchSemester();
